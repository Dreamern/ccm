...
def load_feature_extractor(args, eval=True):
    ...
    if args.loss_norm == 'lpips':
        ...
        feature_extractor = feature_extractor.train(False).cuda()
...
def load_discriminator_and_d_feature_extractor(args):
    ...
    if args.gan_training:
        ...
        discriminator_feature_extractor = discriminator_feature_extractor.train(False).to(args.local_rank)
    ...
    if args.parallel:
        discriminator = torch.nn.parallel.DistributedDataParallel(discriminator, device_ids=[args.local_rank], output_device=args.local_rank, broadcast_buffers=False)
